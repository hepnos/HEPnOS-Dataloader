...
/**
 * Auto-generated rec_trk_discrete_tracks_mrdif_lid_lid structure for the 
 * "/rec.trk.discrete.tracks.mrdif.lid.lid" table in corresponding HDF5 files.
 */
struct rec_trk_discrete_tracks_mrdif_lid_lid {
     ...
     *    (?) f(uint64_t run, uint64_t subrun, uint64_t event, const rec_trk_discrete_tracks_mrdif_lid_lid& value)
     *
    ...
    static void from_hdf5(hid_t file, F&& callback) {
        rec_trk_discrete_tracks_mrdif_lid_lid current;
        hid_t dataset;
        hid_t dataspace;
        hsize_t dims[2];
        herr_t err;
